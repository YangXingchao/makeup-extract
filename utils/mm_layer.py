...
class MakeupModel(nn.Module):
    ...
    def forward(self, x):
        ...
        make = make.reshape(1, 256, 256, 3)
        make = make.permute(0, 3, 1, 2)
        make = F.interpolate(make, [256, 256])
        return make

def get_cropped_mask(device):
    cropped_mask = cv2.imread(config.SKIN_MASK_CROP_PATH)
    cropped_mask = cv2.resize(cropped_mask, (config.UV_TEX_SIZE, config.UV_TEX_SIZE))
    cropped_mask = cv2.cvtColor(cropped_mask, cv2.COLOR_BGR2GRAY)
    _, cropped_mask = cv2.threshold(cropped_mask, 10, 255, cv2.THRESH_BINARY)
    cropped_mask = torch.tensor(cropped_mask.astype(np.float32) / 255.0)[None, None, ...].to(device)
    return cropped_mask