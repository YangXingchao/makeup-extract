...
def compute_mm(shape_layer, tex_layer, spec_layer, tri, id, ex, tx, sp, r, t, s, sh, p, ln, gain, bias):
    v, lm = shape_layer(id, ex)
    albe = tex_layer(tx).permute(0, 2, 3, 1) * gain[:, None, None, :] + bias[:, None, None, :]
    albe = albe.permute(0, 3, 1, 2).clamp(0, 1.)
    spec = spec_layer(sp)
    ...
    mask, diff = diffuse_shading(n_cam, sh, pts=v_cam, cull_thre=0.9)
